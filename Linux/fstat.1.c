...
int recur_dir_st(struct file_ctl*fc, char*path, int lflag, int sflag)
{
    ...
    for(p=fc->pl;p!=NULL;p=p->next)
    {
        ...
        while(d)
        {
            ...
            re=lstat(buf,&fin.st);
            if(re==-1 && !(lflag&LS_IGE)){perror("lstat");goto next;}
...
int out_info(struct file_info*fi,char*path,int sflag)
{
    ...
    if((sflag&(~FG_NAME)&&(sflag&FG_NAME))>0)ind="    ";

    if(sflag&(FG_MODE+FG_SIZE+FG_BLKS+FG_BLKSZ+FG_LINKS+FG_TYPE))
        fnl=1;

    if(sflag&FG_NAME)
        printf("%s\n",fi->name);

    if((sflag&FG_PATH)&&path!=NULL)
        printf("%s%s\n",ind,path);

    if(fnl==1)printf("%s",ind);
    ...
    if(sflag&(FG_UID+FG_GID+FG_DEVID+FG_INODE+FG_RDEVID))fnl=1;
    ...
    if(sflag&FG_TIME)
    {
        printf("%s%s",ind,ctime(&(fi->st.st_atime)));
        printf("%s%s",ind,ctime(&(fi->st.st_mtime)));
        printf("%s%s",ind,ctime(&(fi->st.st_ctime)));
    }

    if(S_ISLNK(fi->st.st_mode)&& (ls_flag&LS_LNK))
    {
        printf("%slink target:","    ");
        char lnkt[LNK_BUF];
        int end=0;
        ...
        char* pa = NULL;
        if(path)pa=(char*)malloc(sizeof(char)*(strlen(path)+strlen(fi->name)+1));
        else pa=(char*)malloc(sizeof(char)*(strlen(fi->name)+1));
        if(!pa)
        {
            perror("malloc");
            return -1;
        ...
        end=readlink(pa,lnkt,LNK_BUF);
        if(end>=LNK_BUF)end=LNK_BUF;
        else if(end<0)
        {
            perror("readlink");
            free(pa);pa=NULL;
...
void help()
{
    ...
    printf("    name\n");
    printf("        path\n");
    printf("        mode;type;size;blocks;blocksize;links\n");
    printf("        uid;gid;major-device;minor-device;inode;rdev\n");
    printf("        time:last [access;modification;change]\n");
