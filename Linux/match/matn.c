...
void get_break(int sig);
char utl(char c);

int main(int ac, char* av[])
{
	signal(SIGINT, get_break);
	signal(SIGQUIT, get_break);
	if(ac<3)
	{
		if(ac<2)
		{
		...
		else
		{
			...
			int k;
			int bl;
			int srclen=strlen(av[1]);
			while(1)
			{
				er=fgets(buf,LINE_MAX+1,stdin);
				if(!er)
					break;
				bl=strlen(buf);
				ix=mat(av[1],buf,mv,srclen);
				if(ix>=0)
				{
					for(k=0;k<bl;k++)
					{
