...
next:
    ...
    if(FG_UID & sflag)
        printf("uid:%d ",fi->st.st_uid);
    if(FG_GID & sflag)
        printf("gid:%d ",fi->st.st_gid);
    if(FG_DEVID & sflag)
        printf("device:%d-%d ",(int)major(fi->st.st_dev),(int)minor(fi->st.st_dev));
    if(FG_INODE & sflag)
        printf("inode:%lld ",(long long)fi->st.st_ino);
    if(FG_RDEVID & sflag)
        printf("rdev:%d",(int)fi->st.st_rdev);
    ...
    if(FG_TIME & sflag)
    {
        printf("Last access time: %s",ctime(&(fi->st.st_atime)));
        printf("Last moltify time: %s",ctime(&(fi->st.st_mtime)));
        printf("Last change time: %s",ctime(&(fi->st.st_ctime)));
    }
